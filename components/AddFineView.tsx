...
export const AddFineView: React.FC<AddFineViewProps> = ({
  ...
  const handleConfirmFines = async () => {
    ...
    try {
      ...
      if (onAddFines) {
        if (!await onAddFines(newFines)) throw new Error('Save not confirmed');
      } else {
        for (const fine of newFines) if (!await onAddFine(fine)) throw new Error('Save not confirmed');
      }
  ...
  return (
    <div className="relative w-full h-full flex-1 min-h-0 flex flex-col">
      ...
      {isConfirmSheetOpen && selectedFine && createPortal(
      ...
      {isAllFinesModalOpen && createPortal(
