...
interface ComplaintModalProps {
  fine: FineEntry;
  onConfirm: (fineId: string, reason: string) => Promise<boolean>;
  onCancel: () => void;
}
...
export const ComplaintModal: React.FC<ComplaintModalProps> = ({ fine, onConfirm, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if(reason.trim()) {
        await runSave(() => onConfirm(fine.id, reason.trim()), onCancel);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="bg-amber-50 px-6 py-4 border-b border-amber-100 flex justify-between items-center">
          ...
          <button onClick={() => { if (!isSaving) onCancel(); }} className="text-amber-800/50 hover:text-amber-800 transition-colors">
            <X size={24} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <fieldset disabled={isSaving} className="p-6 space-y-4">
            ...
            <Button type="submit" fullWidth className="bg-amber-500 hover:bg-amber-600 text-white border-transparent">
                {isSaving ? 'Lagrer …' : 'Send inn klage'}
            </Button>
          </fieldset>
        </form>
      </div>
